  ...
  (let [{::form/keys [master-form form-instance]} env
    ...
    (when visible?
      (let [field-options (fo/get-field-options (comp/component-options form-instance) attr)
            ...
            read-only?    (or (sc-form/read-only? master-form attr) (sc-form/read-only? form-instance attr))
            omit-label?   (form/omit-label? form-instance attr)
            invalid?      (and (not read-only?) (sc-form/invalid-attribute-value? env attr))
            ...
            onSelect      (fn [v] (sc-form/input-changed! env qualified-key v))
  ...
  (let [{::form/keys [form-instance]} env
    ...
    (when visible?
      (let [{::form/keys [attributes] :as form-options} (comp/component-options form-instance)
            ...
            omit-label?        (form/omit-label? form-instance attr)
            top-class          (sufo/top-class form-instance attr)
            validation-message (when invalid? (sc-form/validation-error-message env attr))
              ...
              (ui-wrapped-dropdown
                (merge extra-props
                  {:value    current-selection
                   ...
                   :disabled read-only?
                   :options  options
                   :onChange (fn [v] (sc-form/input-changed! env qualified-key v))}))
                               ...
                               (merge extra-props
                                 {:type     "checkbox"
                                  ...
                                  :onChange #(if-not checked?
                                               (sc-form/input-changed! env qualified-key (vec (conj current-selection value)))
                                               (sc-form/input-changed! env qualified-key (vec (disj current-selection value))))}))
                             (dom/label text))))))
                options))
