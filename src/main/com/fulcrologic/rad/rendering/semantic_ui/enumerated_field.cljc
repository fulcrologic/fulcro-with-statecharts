...
(defn- render-to-many [{::form/keys [form-instance] :as env} {::attr/keys [qualified-key computed-options] :as attribute}]
  (when (sc-form/field-visible? form-instance attribute)
    (let [props        (comp/props form-instance)
          read-only?   (sc-form/read-only? form-instance attribute)
                         ...
                         (dom/input
                           {:type     "checkbox"
                            ...
                            :onChange #(let [selection (if-not checked?
                                         ...
                                         (sc-form/input-changed! env qualified-key selection))})
                         (dom/label text))))))
            options))))))
