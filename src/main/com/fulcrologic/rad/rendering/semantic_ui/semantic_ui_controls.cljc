...
;; Bridge: the new statechart form drives rendering through `sc-form/render-element`,
;; whereas the SUI plugin still ships its renderers via the old map-based registry
;; under `com.fulcrologic.rad.form/element->style->layout`. Delegate by looking up
;; the same renderer the old `fr/render-form :default` would have used.
(defmethod sc-form/render-element :default [renv element]
  (if-let [r (form/render-fn renv element)]
    ((->arity-tolerant r) renv)
    (do
      (log/error "No SUI renderer installed for form element" element)
      nil)))

;; Bridge: the new statechart form's `fr/render-field :default` just logs
;; "No renderer installed". Override here to look up the SUI plugin's
;; per-attribute renderer from the old map registry (`type->style->control`).
(defmethod fr/render-field :default [env attr]
  (if-let [render (form/attr->renderer env attr)]
    ((->arity-tolerant render) env attr)
    (do
      (log/error "No SUI renderer installed for attribute" attr)
      nil)))
