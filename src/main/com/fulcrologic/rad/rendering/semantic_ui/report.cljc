    ...
    (dom/tr {:classes [(when highlighted? "active")]
             :onClick (fn [evt]
                        ...
                        (when-not (false? (suo/get-rendering-options report-instance suo/selectable-table-rows?))
                          ...
                          (sc-report/select-row! report-instance idx)))}
      (map-indexed
      ...
      (div {:className (or
        ...
        (div :.ui.form
          (map-indexed
            ...
            input-layout))
        (when paginate?
          (let [page-count (sc-report/page-count report-instance)]
            (when (> page-count 1)
              (div :.ui.two.column.centered.grid
                (div :.two.wide.column
                  (div {:style {:paddingTop "4px"}}
                    #?(:cljs
                       (sui-pagination/ui-pagination {:activePage   (sc-report/current-page report-instance)
                                                      :onPageChange (fn [_ data]
                                                                      (sc-report/goto-page! report-instance (comp/isoget data "activePage")))
...
(comp/defsc ListReportLayout [this {:keys [report-instance] :as env}]
  ...
  (let [{::report/keys [BodyItem]} (comp/component-options report-instance)
        ...
        rows                    (sc-report/current-rows report-instance)
        loading?                (sc-report/loading? report-instance)]
    (div
    ...
    (dom/table {:className (or sui-table-class "ui selectable table") :classes [table-class]}
                           ...
                           (dom/th {:key     idx
                             ...
                             (if (sortable? column)
                               (dom/a {:onClick (fn [evt]
                                                  (evt/stop-propagation! evt)
                                                  (sc-report/sort-rows! report-instance column))}
                                 label
      ...
      (when (seq rows)
        (dom/tbody
          (map-indexed
            (fn [idx row]
              (let [highlighted-row-idx (sc-report/currently-selected-row report-instance)]
                (render-report-body-item row (merge query-inclusion-props
                                               {:report-instance report-instance
                                                :row-class       BodyItem
        ...
        row-headings     (mapv (fn [{::report/keys [column-heading]
                                 ...
                                 (let [label (or
                                   ...
                                   (if (sortable? attr)
                                     (dom/a {:onClick (fn [evt]
                                                        (evt/stop-propagation! evt)
                                                        (sc-report/sort-rows! report-instance attr))}
                                       label
