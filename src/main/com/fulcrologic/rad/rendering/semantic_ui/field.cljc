...
(defn render-field-factory
  ...
  ([addl-props input-factory]
   (fn [{::form/keys [form-instance] :as env} {::attr/keys [qualified-key] :as attribute}]
     (sc-form/with-field-context [{:keys [value field-style-config
                                          visible? read-only?
                                          validation-message
                                          omit-label?
                                          field-label invalid?]} (sc-form/field-context env attribute)
         ...
         (when visible?
           (div {:key     (str qualified-key)
             ...
             (input-factory (merge addl-props
                              {:value    value
                               :onBlur   (fn [v] (sc-form/input-blur! env qualified-key v))
