...
(defn render-to-many [{::form/keys [form-instance] :as env} {k ::attr/qualified-key :as attr} options]
  (let [{:semantic-ui/keys [add-position]
        ...
        read-only?          (sc-form/read-only? form-instance attr)
        ...
        invalid?            (sc-form/invalid-attribute-value? env attr)
        visible?            (sc-form/field-visible? form-instance attr)
        validation-message  (sc-form/validation-error-message env attr)
                              ...
                              (let [order (if (keyword? add?) add? :append)]
                                (if (?! added-via-upload? env)
                                  ...
                                  (let [possible-types (if (comp/union-component? ui)
                                    ...
                                    (map-indexed
                                      (fn [idx c]
                                        (let [add-child! (fn [_] (sc-form/add-child! form-instance k c {::form/order order}))
                                              add-label  (or
    ...
    (cond
      ...
      (let [possible-forms (if (comp/union-component? ui)
        ...
        (div {:key       (str k)
          ...
          (map-indexed
            (fn [idx ui]
              (let [add-child! (fn [] (sc-form/add-child! form-instance k ui))
                    add-label  (or
...
(defsc SingleFile [this {{::form/keys [form-instance master-form] :as env} :env
  ...
  (let [{::form/keys [ui title can-delete? can-add?]} (fo/subform-options options attr)
        ...
        form-props (comp/props form-instance)
        read-only? (or
                     (sc-form/read-only? master-form attr)
                     (sc-form/read-only? form-instance attr))
        add?       (if read-only? false (?! can-add? form-instance attr))
        props      (get form-props k)
...
(defn render-attribute [env attr options]
  (cond
    (or
      ...
      (fro/style attr)) (sc-form/render-field env attr)
    ...
    :else (sc-form/render-field env attr)))
  ...
  (ui-modal {:open open?}
    ...
    (ui-modal-actions {}
      (dom/button :.ui.button
        {:onClick (fn [] (sc-form/trigger! form-instance :event/clear-route-denied))}
        (tr "Return to Editing"))
      (dom/button :.ui.button
        {:onClick (fn [] (sc-form/trigger! form-instance :event/continue-abandoned-route))}
        (tr "Abandon Changes")))))
...
(defsc StandardFormContainer [this {::form/keys [props computed-props form-instance master-form] :as env}]
  ...
  (let [{::form/keys [can-delete?]} computed-props
        ...
        invalid?        (if read-only-form? false (sc-form/invalid? env))
    ...
    (when #?(:cljs goog.DEBUG :clj true)
      (let [valid? (if read-only-form? true (sc-form/valid? env))
        ...
        (div :.ui.form {:classes [(when errors? "error")]
          ...
          (when can-delete?
            (button :.ui.icon.primary.right.floated.button {:disabled (not can-delete?)
                                                            :onClick  (fn [] (sc-form/delete-child! env))}
              (i :.times.icon)))
          (render-fields env)))
              ...
              (when (seq errors)
                (div :.ui.red.message
                  (div :.content
                    ...
                    (when-not new?
                      (dom/a {:onClick (fn []
                                         (sc-form/undo-via-load! env))} (tr "Reload from server"))))))
...
(defn- file-icon-renderer* [{::form/keys [form-instance] :as env}]
  (let [{::form/keys [attributes] :as options} (comp/component-options form-instance)
        ...
        invalid?  (sc-form/invalid-attribute-value? env attribute)
